...
ASSERT_MSG_VA(expr->kind == expect, "expected '%s' got '%s'", describe_ast_kind(expect), describe_ast(expr))
...
ASSERT_MSG_VA(!test_package.errors, "Parsing produced error: '%s'", test_package.errors[0].msg); \
ASSERT_MSG_VA(stmt->kind == expect, "expected '%s' got '%s'", describe_ast_kind(expect), describe_ast(expr))
